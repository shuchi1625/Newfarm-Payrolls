...
def load_data():
    ...
    try:
        conn = psycopg2.connect(
            ...
            password=os.environ.get("DB_PASSWORD", "@rSHUCHI16"),
...
def create_slicing_charts(df):
    ...
    # Bar chart: Total jobs created for every year in selected range
    yearly_jobs = (
        df_avg_jobs.groupby(df_avg_jobs['date'].dt.year)['total_nonfarm']
        .sum()
        .reset_index()
        .rename(columns={'date': 'Year', 'total_nonfarm': 'Total Jobs Created'})
    )
    fig_bar = px.bar(
        yearly_jobs,
        x='Year',
        y='Total Jobs Created',
        title=f"Total Jobs Created Per Year ({year_range[0]}-{year_range[1]})",
        labels={'Year': 'Year', 'Total Jobs Created': 'Total Jobs Created'},
        color_discrete_sequence=['#B57EDC']  # Lavender
    )
    fig_bar.update_layout(xaxis=dict(dtick=1))
    st.plotly_chart(fig_bar, use_container_width=True)

    # Section: Presidents of the USA during selected interval
    presidents = [
    ...
    selected_presidents = [
        p["name"] for p in presidents
        if not (p["end"] < year_range[0] or p["start"] > year_range[1])
    ]
    presidents_text = ", ".join(selected_presidents)
    st.text_input(
        f"Presidents of the USA ({year_range[0]}-{year_range[1]})",
        value=presidents_text,
        disabled=True
    )
    ...
    # Add year range slider for this chart
    min_year_slice2 = int(df['date'].dt.year.min())
    max_year_slice2 = int(df['date'].dt.year.max())
    year_range_slice2 = st.slider(
        "Select year range for monthly comparison:",
        min_value=min_year_slice2,
        max_value=max_year_slice2,
        value=(2019, 2020),
        step=1
    )
    ...
    df_slice2 = df[
        (df['year'] >= year_range_slice2[0]) & 
        (df['year'] <= year_range_slice2[1]) & 
        (df['month'].between(3, 12))
    ]
    # Generate a color map for each year in the selected range
    # Remove matplotlib dependency, use Plotly's default or custom colors
    plotly_colors = [
        "#636EFA", "#EF553B", "#00CC96", "#AB63FA", "#FFA15A",
        "#19D3F3", "#FF6692", "#B6E880", "#FF97FF", "#FECB52"
    ]
    years_in_range = sorted(df_slice2['year'].unique())
    color_map = {year: plotly_colors[i % len(plotly_colors)] for i, year in enumerate(years_in_range)}
    fig2 = px.line(
        df_slice2,
        x='date',
        y='total_nonfarm',
        color='year',
        title=f"Monthly Employment: March-December {year_range_slice2[0]} to {year_range_slice2[1]}",
